import os
import gc
from ctgan.utils import load_tensor_data_v3, get_transformer_v3, add_padding, merge_training_hist, get_training_hist, save_latest_training_info, save_training_history, save_model_weights, get_df, get_colname_df, load_model_weights
...
def proceed_finetune(list_data_paths, configs, model_config, model, model_type, data_path, save_path, pretrain_path):
    if model_type in ['ctgan', 'tvae', 'stvae', 'stvaem']:
        _proceed_finetune_based_ctgan_tvae(list_data_paths, configs, model_config, model, model_type, data_path, save_path, pretrain_path)

    if model_type in ['great']:
        _proceed_finetune_based_great(list_data_paths, configs, model_config, model, model_type, data_path, save_path, pretrain_path)

def _proceed_finetune_based_ctgan_tvae(list_data_paths, configs, model_config, model, model_type, data_path, save_path, pretrain_path):
    DATA_PATH = data_path
    SAVE_PATH = save_path
...
def _proceed_finetune_based_great(list_data_paths, configs, model_config, model, model_type, data_path, save_path, pretrain_path):
    PRETRAIN_PATH = pretrain_path
    DATA_PATH = data_path
    SAVE_PATH = save_path
    training_hist = []
